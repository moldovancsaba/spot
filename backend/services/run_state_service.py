...
def _resolve_run_state(*, existing_state: str | None, progress: dict | list | None, control: dict | list | None) -> str:
    ...
    return str(existing_state or "UNKNOWN")
...
def refresh_run_record(*, runs_dir: Path, run_id: str) -> dict | None:
    ...
    record["state"] = _resolve_run_state(existing_state=record.get("state"), progress=progress, control=control)
    ...
    return write_run_record(runs_dir=runs_dir, run_id=run_id, record=record)
...
def build_run_detail(*, runs_dir: Path, run_id: str) -> dict | None:
    ...
    available_operations = {
        ...
        "cancel": running or state in {"STARTING", "PENDING", "VALIDATING", "PROCESSING", "WRITING", "PAUSED"},
