...
Reviewed active runtime and browser-operator paths:
...
- [`backend/routes/ui.py`](/Users/moldovancsaba/Projects/spot/backend/routes/ui.py)
...
2. Active browser/operator docs had drifted from the implemented intake contract and queue-backed dashboard behavior.
...
- Aligned active current-state docs to the implemented queue-backed browser dashboard, local segmentation model, and main-page pause/resume/stop controls.
- Aligned executable smoke coverage to the current intake contract and current browser-surface title strings.

## Result

No critical stale code comments remain in the actively reviewed runtime and browser-operator paths covered by this audit pass.
